...
def ElipseRect(x0, y0, z0, Lx, Ly,vec_z, vec_x, x_eps=1e-6,factor=2.0):
    ...
    p1_major = np.array([-factor*Lx/2 , 0.0, 0.0])
    p2_major = np.array([+factor*Lx/2 , 0.0, 0.0])
    ...
    p1_major = np.dot(rot, p1_major)
    p2_major = np.dot(rot, p2_major)
    ...
    p1_major = p1_major + np.array([x0, y0, z0])
    p2_major = p2_major + np.array([x0, y0, z0])
    ...
    p1_major = gmsh.model.occ.addPoint(*p1_major)
    p2_major = gmsh.model.occ.addPoint(*p2_major)

    gmsh.model.occ.synchronize()
    gmsh.fltk.run()

    arc1 = gmsh.model.occ.addEllipseArc(p1y1, p1,p1_major, p1y2)
    # ly2  = gmsh.model.occ.addLine(p1y2,p2y2)
    ly2_1 = gmsh.model.occ.addLine(p1y2,y2_mid)
    ly2_2 = gmsh.model.occ.addLine(y2_mid,p2y2)

    arc2 = gmsh.model.occ.addEllipseArc(p2y2, p2,p2_major,p2y1)
